...
class EnvObject:
    ...
    def model_object_env_init(self, model_set: dict) -> None: #more than just dict
        ...
        for model_name in model_set:
            ...
            model_obj_path_save = os.path.join(model_obj_path, model_name)
            ...
            self.paths_of_model_objects.append(model_obj_path)

            if model_obj_exists: continue

            create_fodler(model_obj_path)
            model_set[model_name].save(model_obj_path)

    
    def init_model_objects_list(self) -> None:
        object_parent = os.path.join(self.model_path, 'models')
        self.model_objects = [model_name for model_name in os.listdir(object_parent)]
